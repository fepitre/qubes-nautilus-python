...
FEDORA_SOURCES := https://src.fedoraproject.org/rpms/nautilus-python/raw/master/f/sources
SRC_FILE := nautilus-python-$(VERSION).tar.xz

BUILDER_DIR ?= ../..
SRC_DIR ?= qubes-src

DISTFILES_MIRROR ?= https://download.gnome.org/sources/nautilus-python/$(subst .$(word 3,$(subst ., ,$(VERSION))),,$(VERSION))/
